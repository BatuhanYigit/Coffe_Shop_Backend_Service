...
join district d on d.id=districtid
where u.id={userid}"""

get_basket_detail = """select bd.basket_id,bd.totalprice,i.item_name,i.price,s.sugar_type,s.sugar_price,m.milk_type,m.milk_price,sy.syrup_type,sy.syrup_price,sz.size,sz.size_price
from basketdetail bd
join item i  on i.id=bd.item_id
join milk m on m.id = bd.milk_id
join syrup sy on sy.id = bd.syrup_id
join sugar s on s.id = bd.sugar_id
join size sz on sz.id = bd.size_id
where bd.id={basket_id}"""
